...
# ── approve_plan.sh ───────────────────────────────────────────────────────────
# Plan-approval gate. The daemon pipes a task's `plan` text to this script before
# executing; exit 0 approves, exit 2 rejects with the stderr text sent back to
# Cowork. Ships as a no-op that only logs to plan_log.jsonl — uncomment the
# BLOCKED_PATTERNS / notification / interactive sections to enforce a policy.
cat > "$BRIDGE_ROOT/scripts/approve_plan.sh" <<'APPROVEPLAN'
#!/usr/bin/env bash
...
exit 0
APPROVEPLAN
chmod +x "$BRIDGE_ROOT/scripts/approve_plan.sh"
