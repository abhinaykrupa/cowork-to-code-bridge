"""
mcp_operations.py — operation-lifecycle MCPServer for the bridge.

Exposes the bridge as an MCP *model provider*: Hermes, Open Claw, and other
agents connect and escalate work to Claude Code, then track and cancel those
long-running operations.  This is distinct from ``mcp_server.py``, which is the
stdio JSON-RPC *proxy* that forwards tool calls to the daemon.

Tools:
  • escalate_to_claude   — hand a request to Claude Code, poll for a reply
  • run_script           — execute a whitelisted script directly (no agent)
  • list_bridge_scripts  — discover available whitelisted scripts
  • get_operation_status — check a long-running escalation (idempotent), with
                           per-operation metrics + loop detection
  • cancel_operation     — cancel a queued/running operation (SIGTERM best-effort)

The class is import-safe (stdlib + ``client.call_remote_streaming`` only) so it
