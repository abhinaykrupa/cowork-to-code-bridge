...
call_remote / queue_task, e.g. `call_remote("scripts/mac_ram.sh")`. This list is
what a stock install has; `list_scripts.sh` reports what is actually present
(the user may have added their own).

`run_claude.sh`
    Hands a task to Claude Code on the machine — the main event, a real local
    agent plans and iterates. Slow: queue_task it rather than blocking.
`list_scripts.sh`
    Every script this bridge can run, with descriptions. `--json` for a
    parseable catalog. Call this to discover what's there — don't guess.
`ping.sh` / `hello.sh`
    Confirm the bridge works. Start here if anything looks wrong.
`mac_health.sh`
    Full snapshot: CPU, memory, disk, battery, top procs. `--json`.
`mac_ram.sh`
    RAM usage. `--json` → `{total,free,used}_bytes`.
`mac_disk.sh`
    Disk space. `--json` → `{path,total_1k,used_1k,avail_1k,used_pct}`.
`mac_top.sh`
    Top processes by CPU and memory. `--json` → `{count,by_cpu[],by_mem[]}`.
`mac_network.sh`
    Network status and connectivity.
    `--json` → `{interfaces[],default_route,online}`.
`disk_hogs.sh`
    Biggest files/folders in a path. Args: dir, optional count. `--json`.
`port_check.sh`
    What is listening on a TCP port. Args: port. `--json`.
`process_kill.sh`
    Terminate a named process or PID. SIGTERM only, refuses protected and
    kernel processes. `--all` for multi-match, `--json`.
`docker_ps.sh`
    Running Docker containers. `--json` → `{ok,error,containers[]}`.
`docker_logs.sh`
    Tail a container's logs. Args: container, optional line count (default 50).
`git_status.sh`
    `git status --short --branch` in a repo. Args: repo path. `--json`.
`pkg_outdated.sh`
    Outdated packages (brew / apt / dnf / yum / pacman).
    `--json` → `{manager,count,packages[]}`.
`env_check.sh`
    Key env values (PATH, BRIDGE_ROOT, CLAUDE_FLAGS, `claude` CLI). Never
    leaks the token. `--json`.
`open_browser.sh`
    Opens an http(s)/localhost URL in the default browser. Handy right after
    a dev server starts.
`mcp_audit.sh`
    JSON snapshot of MCP servers registered in the local Claude Code, so you
    can diff them against what you can reach from here.
`mcp_list_servers.sh`
    Lists local MCP servers.
`mcp_register.sh`
    Registers an MCP server with the local Claude Code.
`mcp_proxy.sh`
    Relays JSON-RPC to a local stdio MCP server — lets you drive an MCP that
...
While "running" the dict also carries a live ticker from the daemon's status
file (written every ~2s): elapsed_s, last_line, state, plus a pre-rendered
status_line like "⣾ Working… 42s elapsed" — show it to the user instead of
going silent between polls.

### call_remote_streaming(script, ..., on_progress=None, on_status=None)  — BLOCKING, LIVE OUTPUT
Like call_remote, but streams the script's output while it runs. on_progress
receives each new chunk of stdout/stderr text; on_status receives the daemon's
~2s status dicts ({elapsed_s, last_line, state}) — feed them to
...
- While a task is "running", surface poll_task_result's **status_line** (or use
  call_remote_streaming's on_status) so the user sees a live spinner, not silence.
